...
import (
	"encoding/json"
	"fmt"
	"github.com/FISCO-BCOS/go-sdk-ios/wrappergen/tamplate"
	"io/ioutil"
	"os"
	"path"
	"path/filepath"
...
func init() {
	...
	app.Action = utils.MigrateFlags(warappergen)
	cli.CommandHelpTemplate = utils.OriginCommandHelpTemplate
}

func warappergen(c *cli.Context) error {
	utils.CheckExclusive(c, abiFlag, jsonFlag, solFlag, vyFlag) // Only one source can be selected.
	if c.GlobalString(pkgFlag.Name) == "" {
		utils.Fatalf("No destination package specified (--pkg)")
	}
	var lang tamplate.LangType
	switch c.GlobalString(langFlag.Name) {
	case "go":
		lang = tamplate.LangGo
	case "java":
		lang = tamplate.LangJava
	case "objc":
		lang = tamplate.LangObjC
	default:
	...
	code, err := tamplate.Bind(types, abis, bins, sigs, c.GlobalString(pkgFlag.Name), lang, libs, aliases, smcrypto)
	if err != nil {
	...
	if lang == tamplate.LangObjC {
		code, err := tamplate.Bind(types, abis, bins, sigs, c.GlobalString(pkgFlag.Name), tamplate.LangObjCHeader, libs, aliases, smcrypto)
		if err != nil {
			utils.Fatalf("Failed to generate ABI binding: %v", err)
		}
		// Either flush it out to a file or display on the standard output
