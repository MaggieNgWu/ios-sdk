...
var tmplSource = map[LangType]string{
	LangGo:         tmplSourceGo,
...
const tmplSourceObjc = `{{range $contract := .Contracts}}@implementation {{.Type}}
	// {{.Type}}ABI is the input ABI used to generate the binding from.
	NSString * address;
	NSString * abi = @"{{.InputABI}}"
	NSString * bin = @"0x{{.InputBin}}";
	...
	- (Transaction*) deploy {{range .Constructor.Inputs}}:(NSString *) {{.Name}}{{end}}{
		return deployContract((char *)[abi cStringUsingEncoding:NSUTF8StringEncoding], (char *)[bin cStringUsingEncoding:NSUTF8StringEncoding]);
	}

	{{range .Calls}}
	- (CallResult *){{.Normalized.Name}}{{range .Normalized.Inputs}}:(NSString *) {{.Name}}{{end}}{
		{{if .Normalized.Inputs}}
		NSString * params = [NSString stringWithFormat:@"[{{range $i, $_ := .Normalized.Inputs}}{{if ne $i 0}}*{{else}}{{end}}{\"type\":\"{{.Type}}\",\"value\":\"%@\"}{{end}}]" {{range .Normalized.Inputs}},{{.Name}}{{end}}];{{else}}NSString * param = @"[]"{{end}}
		return call((char *)[abi cStringUsingEncoding:NSUTF8StringEncoding],(char *)[address cStringUsingEncoding:NSUTF8StringEncoding],(char *)[@"{{.Normalized.Name}}" cStringUsingEncoding:NSUTF8StringEncoding],(char *)[param cStringUsingEncoding:NSUTF8StringEncoding]);
	}
	{{end}}
 
	{{range .Transacts}}
	- (TxExecResult *){{.Normalized.Name}}{{range .Normalized.Inputs}}:(NSString *) {{.Name}}{{end}}{
		{{if .Normalized.Inputs}}
		NSString * params = [NSString stringWithFormat:@"[{{range $i, $_ := .Normalized.Inputs}}{{if ne $i 0}}*{{else}}{{end}}{\"type\":\"{{.Type}}\",\"value\":\"%@\"}{{end}}]" {{range .Normalized.Inputs}},{{.Name}}{{end}}];{{else}}NSString * param = @"[]"{{end}}
		return sendTransaction((char *)[abi cStringUsingEncoding:NSUTF8StringEncoding],(char *)[address cStringUsingEncoding:NSUTF8StringEncoding],(char *)[@"{{.Normalized.Name}}" cStringUsingEncoding:NSUTF8StringEncoding],(char *)[param cStringUsingEncoding:NSUTF8StringEncoding]);
	}
	{{end}}
...
- (instancetype)initWithAddress:(NSString *)addr;
- (Transaction*) deploy {{range .Constructor.Inputs}}:(NSString *) {{.Name}}{{end}};
{{range .Calls}}- (CallResult *){{.Normalized.Name}}{{range .Normalized.Inputs}}:(NSString *) {{.Name}}{{end}};{{end}}
{{range .Transacts}}- (TxExecResult *){{.Normalized.Name}}{{range .Normalized.Inputs}}:(NSString *) {{.Name}}{{end}};{{end}}
